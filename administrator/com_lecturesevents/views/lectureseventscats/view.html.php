...
class LecturesEventsCpViewLecturesEventsCats extends JView
{
	protected $items;
...
//		}
	...
	protected function addToolbar() {
		...
		require_once JPATH_COMPONENT.DS.'helpers'.DS.'lectureseventscats.php';

		$state	= $this->get('State');
		$canDo	= LecturesEventsCatsHelper::getActions($state->get('filter.category_id'));
		
		JToolBarHelper::title( JText::_( 'COM_LECTURESEVENTS_CATEGORIES' ), 'category.png' );
		if ($canDo->get('core.create')) {
			JToolBarHelper::addNew('lectureseventscat.add','JTOOLBAR_NEW');
		}
		if ($canDo->get('core.edit')) {
			JToolBarHelper::editList('lectureseventscat.edit','JTOOLBAR_EDIT');
		}
		if ($canDo->get('core.edit.state')) {

			JToolBarHelper::divider();
			JToolBarHelper::custom('lectureseventscats.publish', 'publish.png', 'publish_f2.png','JTOOLBAR_PUBLISH', true);
			JToolBarHelper::custom('lectureseventscats.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
			//JToolBarHelper::custom( 'lectureseventscats.approve', 'approve.png', '', 'COM_LECTURESEVENTS_APPROVE' , true);
			//JToolBarHelper::custom( 'lectureseventscats.disapprove', 'disapprove.png', '',  'COM_LECTURESEVENTS_NOT_APPROVE' , true);
		}

		if ($canDo->get('core.delete')) {
			JToolBarHelper::deleteList( JText::_( 'COM_LECTURESEVENTS_WARNING_DELETE_ITEMS' ), 'lectureseventscats.delete', 'COM_LECTURESEVENTS_DELETE');
		}
		JToolBarHelper::divider();
