...
class LecturesEventsCpControllerLecturesEventsUpload extends LecturesEventsCpController
{
	...
	function createfolder() {
		...
		JRequest::checkToken() or jexit( 'COM_LECTURESEVENTS_INVALID_TOKEN' );
		...
		$viewBack		= JRequest::getVar( 'viewback', '', '', 'lectureseventsmanager' );
		...
		if ($manager != '') {
			...
			$link	= 'index.php?option=com_lecturesevents&view='.(string)$viewBack.'&manager='.(string)$manager
			...
			$app->redirect('index.php?option=com_lecturesevents', JText::_('COM_LECTURESEVENTS_ERROR_CONTROLLER_MANAGER_NOT_SET'));
			exit;
		}
		...
		if (($folderCheck !== null) && ($folderNew !== $folderCheck)) {
			$app->redirect($link, JText::_('COM_LECTURESEVENTS_WARNING_DIRNAME'));
		}
		...
		if (strlen($folderNew) > 0) {
			...
			if (!JFolder::exists($folder) && !JFile::exists($folder)) {
				...
				if (isset($folder)) {
					...
					$app->redirect($link, JText::_('COM_LECTURESEVENTS_ERROR_FOLDER_CREATING'));
				}
				
				$app->redirect($link, JText::_('COM_LECTURESEVENTS_SUCCESS_FOLDER_CREATING'));
			} else {
				$app->redirect($link, JText::_('COM_LECTURESEVENTS_ERROR_FOLDER_CREATING_EXISTS'));
			}
	...
	function multipleupload() {
		$result = LecturesEventsFileUpload::realMultipleUpload();
		return true;	
	}
	
	function upload() {
		$result = LecturesEventsFileUpload::realSingleUpload();
		return true;
	}
