...
class LecturesEventsCpModelLecturesEventsRaFile extends JModelList
{
...
*/
	...
	protected function getListQuery()
	{
		...
		$query = ' SELECT a.*, cc.title AS categorytitle, cc.id AS categoryid, i.title AS imagetitle, i.id AS imageid, ua.name AS editor, u.id AS ratinguserid, u.username AS ratingusername '
			. ' FROM #__lecturesevents_file_votes AS a '
			. ' LEFT JOIN #__lecturesevents AS i ON i.id = a.imgid '
			. ' LEFT JOIN #__lecturesevents_categories AS cc ON cc.id = i.catid '
	...
	function delete($cid = array()) {
		...
		if (count( $cid )) {
			...
			$query = 'SELECT v.fileid AS imgid'
				. ' FROM #__lecturesevents_file_votes AS v'
				. ' WHERE v.id IN ( '.$cids.' )';
			$images = $this->_getList($query);
			
			//Delete it from DB
			$query = 'DELETE FROM #__lecturesevents_file_votes'
				. ' WHERE id IN ( '.$cids.' )';
			...
			foreach ($images as $valueImgId) {
				$updated = LecturesEventsRateHelper::updateVoteStatisticsFile( $valueImgId->imgid );
				if(!$updated) {
					return false;
				}				
			}
	...
	protected function prepareTable(&$table)
	{
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__lecturesevents_file_votes');
				$max = $db->loadResult();
