...
class LecturesEventsCpModelLecturesEventsCat extends JModelAdmin
{
	...
	protected function canDelete($record)
	{
		...
		if ($record->catid) {
			return $user->authorise('core.delete', 'com_lecturesevents.lectureseventscat.'.(int) $record->catid);
		} else {
	...
	protected function canEditState($record)
	{
		...
		if ($record->catid) {
			return $user->authorise('core.edit.state', 'com_lecturesevents.lectureseventscat.'.(int) $record->catid);
		} else {
			return parent::canEditState($record);
		}
	}
	
	public function getTable($type = 'LecturesEventsCat', $prefix = 'Table', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true) {
		...
		$form 	= $this->loadForm('com_lecturesevents.lectureseventscat', 'lectureseventscat', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
		return $form;
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_lecturesevents.edit.lectureseventscat.data', array());
	...
	protected function prepareTable(&$table)
	{
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__lecturesevents_categories');
				$max = $db->loadResult();
	...
	function delete($cid = array()) {
		...
		if (count( $cid )) {
			...
			. ' LEFT JOIN #__lecturesevents_categories AS s ON s.parent_id = c.id'
			. ' WHERE c.id IN ( '.$cids.' )'
			. ' GROUP BY c.id'
			;
			$db->setQuery( $query );
			...
			if (count( $cid )) {
				...
				// Select id's from lecturesevents tables. If the category has some images, don't delete it
				$query = 'SELECT c.id, c.name, c.title, COUNT( s.catid ) AS numcat'
				. ' FROM #__lecturesevents_categories AS c'
				. ' LEFT JOIN #__lecturesevents AS s ON s.catid = c.id'
				. ' WHERE c.id IN ( '.$cids.' )'
				. ' GROUP BY c.id';
				...
				if (count( $cid )) {
					$cids = implode( ',', $cid );
					$query = 'DELETE FROM #__lecturesevents_categories'
					. ' WHERE id IN ( '.$cids.' )';
					$db->setQuery( $query );
					if (!$db->query()) {
					...
					// Delete items in lecturesevents_user_category
				/*	$query = 'DELETE FROM #__lecturesevents_user_category'
					. ' WHERE catid IN ( '.$cids.' )';
					$db->setQuery( $query );
					if (!$db->query()) {
						$this->setError($this->_db->getErrorMsg());
			...
			if (count( $err_cat ) || count( $err_img )) {
				if (count( $err_cat )) {
					...
					$msg .= JText::plural( 'COM_LECTURESEVENTS_ERROR_DELETE_CONTAIN_CAT', $cids_cat );
				}
				
				if (count( $err_img )) {
					$cids_img = implode( ", ", $err_img );
					$msg .= JText::plural( 'COM_LECTURESEVENTS_ERROR_DELETE_CONTAIN_FILE', $cids_img );
				}
	...
	public function increaseOrdering($categoryId) {
		...
		$ordering = 1;
		$this->_db->setQuery('SELECT MAX(ordering) FROM #__lecturesevents_categories WHERE parent_id='.(int)$categoryId);
