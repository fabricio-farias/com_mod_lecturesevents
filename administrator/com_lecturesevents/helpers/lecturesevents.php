...
{	
	function CategoryTreeOption($data, $tree, $id=0, $text='', $currentId) {		
		...
		foreach ($data as $key) {	
			...
			if ($key->parentid == $id && $currentId != $id && $currentId != $key->value) {
				...
				$tree = LecturesEventsHelper::CategoryTreeOption($data, $tree, $key->value, $show_text . " - ", $currentId );	
			}	
		}
		return($tree);
	}
		...
		if ( $nouser ) {
			...
			switch ($name) {
				case 'jform[accessuserid][]':
					$idInput1 	= -1;
					$idText1	= JText::_( 'COM_LECTURESEVENTS_ALL_REGISTERED_USERS' );
					$idInput2 	= -2;
					$idText2	= JText::_( 'COM_LECTURESEVENTS_NOBODY' );
				break;
				
				Default:
					$idInput1 	= -2;
					$idText1	= JText::_( 'COM_LECTURESEVENTS_NOBODY' );
					$idInput2 	= -1;
					$idText2	= JText::_( 'COM_LECTURESEVENTS_ALL_REGISTERED_USERS' );
				break;
			}
	...
	function filterCategory($query, $active = NULL, $frontend = NULL, $onChange = TRUE, $fullTree = NULL ) {
		...
		$categories[] = JHTML::_('select.option', '0', '- '.JText::_('COM_LECTURESEVENTS_SELECT_CATEGORY').' -');
		$db->setQuery($query);
		...
		if ($fullTree) {
			...
			$queryAll = ' SELECT cc.id AS value, cc.title AS text, cc.parent_id as parentid'
					.' FROM #__lecturesevents_categories AS cc'
					.' ORDER BY cc.ordering';
			$db->setQuery($queryAll);
			$catDataAll 		= $db->loadObjectList();

			$catDataTree	= LecturesEventsHelper::CategoryTreeOption($catDataAll, $tree, 0, $text, -1);
	...
	function filterSection($query, $active = NULL, $frontend = NULL) {
		...
		$sections[] = JHTML::_('select.option', '0', '- '.JText::_('LECTURESEVENTS_SELECT_SECTION').' -');
		$db->setQuery( $query );
	...
	*/
	...
	function getTitleFromFilenameWithoutExt (&$filename) {
		...
		if ($string) {
			return LecturesEventsHelper::removeExtension($folder_array[$last_array_value]);
		} else {
	...
	function canPlay( $fileName ) {
		$fileExt 	= LecturesEventsHelper::getExtension($fileName);
	...
	function getPathSet( $manager = '') {
		...
		$downloadFolder		= $paramsC->get( 'download_folder', 'lecturesevents' );
		$downloadFolderPap	= $paramsC->get( 'download_folder_pap', 'lectureseventspap' );
		...
		if ($group['f'] == 2) {
			// Images
			$path['orig_abs'] 				= JPATH_ROOT . DS . 'images' . DS . 'lecturesevents' ;
			$path['orig_abs_ds'] 			= $path['orig_abs'] . DS ;
			$path['orig_abs_user_upload'] 	= '';//$path['orig_abs'] . DS . 'userupload' ;
			$path['orig_rel_ds'] 			= '../images/lecturesevents/';
		} else if ($group['f'] == 3) {
			// Play and Preview
	...
	function getLectVersion()
	{
		$folder = JPATH_ADMINISTRATOR .DS. 'components'.DS.'com_lecturesevents';
		if (JFolder::exists($folder)) {
			$xmlFilesInDir = JFolder::files($folder, '.xml$');
		} else {
			$folder = JPATH_SITE .DS. 'components'.DS.'com_lecturesevents';
			if (JFolder::exists($folder)) {
				$xmlFilesInDir = JFolder::files($folder, '.xml$');
			} else {
				$xmlFilesInDir = null;
	...
	function getFileSize($filename, $readable = 1) {
		...
		$path			= &LecturesEventsHelper::getPathSet();
		$fileNameAbs	= JPath::clean($path['orig_abs'] . DS . $filename);
		
		if ($readable == 1) {
			return LecturesEventsHelper::getFileSizeReadable(filesize($fileNameAbs));
		} else {
	...
	function getSettings($title = '', $default = '') {
		...
		$query = ' SELECT '.$select
			. ' FROM #__lecturesevents_settings AS st'
			. $where
			. ' ORDER BY st.id';
	...
	function getSelectSettings($id, $title, $value, $values, $class = 'inputbox', $size = 50, $maxlength = 255, $style = 'width:300px' ) {
		...
		$valuesArray = LecturesEventsHelper::getSettingsValues($values);
	...
	*/
	function displayNewIcon ($date, $time = 0) {
		...
		if ($dateExists < $dateNew) {
			return '&nbsp;'. JHTML::_('image', 'components/com_lecturesevents/assets/images/icon-new.png', JText::_('COM_LECTURESEVENTS_NEW'));
		} else {
			return '';
		}
	...
	function displayHotIcon ($hits, $requiredHits = 0) {
		...
		if ($requiredHits <= $hits) {
			return '&nbsp;'. JHTML::_('image', 'components/com_lecturesevents/assets/images/icon-hot.png', JText::_('COM_LECTURESEVENTS_HOT'));
		} else {
			return '';
		}
	...
	function approved( &$row, $i, $imgY = 'tick.png', $imgX = 'publish_x.png', $prefix='' ) {
		...
		$alt 	= $row->approved ? JText::_( 'LECTURESEVENTS_APPROVED' ) : JText::_( 'LECTURESEVENTS_NOT_APPROVED' );
		$action = $row->approved ? JText::_( 'LECTURESEVENTS_DISAPPROVE_ITEM' ) : JText::_( 'LECTURESEVENTS_APPROVE_ITEM' );
	...
	function getCategoryAccess($id) {
		...
		$query 	= 'SELECT c.access, c.uploaduserid, c.deleteuserid' .
				' FROM #__lecturesevents_categories AS c' .
				' WHERE c.id = '. (int) $id;
	...
	function getCategoryAccessByFileId($id) {
		...
		$query 	= 'SELECT c.access, c.uploaduserid, c.deleteuserid' .
				' FROM #__lecturesevents_categories AS c' .
				' LEFT JOIN #__lecturesevents as a ON a.catid = c.id' .
				' WHERE a.id = '. (int) $id;
		$db->setQuery($query, 0, 1);
		$output = $db->loadObject();
		return $output;
	 ...
	 */
	function sendLecturesEventsMail ( $id, $fileName, $method = 1 ) {
		$app = JFactory::getApplication();
	...
	public function displayMirrorLinks($view = 1, $link, $title, $target) {
		...
		if ($displayM == 1 && $link != '' && LecturesEventsHelper::isURLAddress($link) && $title != '') {
	...
	public function displayReportLink($view = 1, $title = '') {
		...
		if ($displayL == 1 && (int)$param['report_link_guestbook_id'] > 0) {
			...
			$o .= '<a href="'.$href.'" onclick="'.$onclick.'">'.JText::_('COM_LECTURESEVENTS_REPORT').'</a>';
	...
	public function getLayoutText($type) {
		...
		$query = 'SELECT a.'.$type
		.' FROM #__lecturesevents_layout AS a';
