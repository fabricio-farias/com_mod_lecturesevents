...
class LecturesEventsFileUpload
{
	public function realMultipleUpload( $frontEnd = 0) {
		...
		JRequest::checkToken( 'request' ) or jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'error', 'code' => 100,
				'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
				'details' => JTEXT::_('COM_LECTURESEVENTS_INVALID_TOKEN'))));
		...
		$path	= LecturesEventsHelper::getPathSet($manager);// we use viewback to get right path
		...
		if (isset($file['name'])) {
			...
			if ($chunkEnabled == 1) {
				...
				if (JFile::exists($filepathImgFinal)) {
					if($lastChunk == $chunks){
						...
						jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'error', 'code' => 108,
							'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
							'details' => JTEXT::_('COM_LECTURESEVENTS_FILE_ALREADY_EXISTS'))));
					
				}
			
				if (!LecturesEventsFileUpload::canUpload( $file, $errUploadMsg, $manager, $frontEnd, $chunkEnabled, $realSize )) {
				...
				if (JFolder::exists($filepathFolderTemp)) {
					...
					jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'error', 'code' => 100,
							'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
							'details' => JTEXT::_('COM_LECTURESEVENTS_ERROR_FOLDER_UPLOAD_NOT_EXISTS'))));
				}
					...
					if (isset($file['tmp_name']) && is_uploaded_file($file['tmp_name'])) {
						...
						if ($out) {
							...
							if ($in) {
								...
								'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
								'details' => JTEXT::_('COM_LECTURESEVENTS_ERROR_OPEN_INPUT_STREAM'))));
							}
							$stream->close();
							//fclose($out);
							...
							'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
							'details' => JTEXT::_('COM_LECTURESEVENTS_ERROR_OPEN_OUTPUT_STREAM'))));
						}
					} else {
						jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'error', 'code' => 103,
							'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
							'details' => JTEXT::_('COM_LECTURESEVENTS_ERROR_MOVE_UPLOADED_FILE'))));
					}
				} else {
					// Open temp file
					$out = $stream->open($filepathImgTemp, $chunk == 0 ? "wb" : "ab");
				...
				if($lastChunk == $chunks){
					...
					/*if(($imginfo = getimagesize($filepathImgTemp)) === FALSE) {
						...
						'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
						'details' => JTEXT::_('COM_LECTURESEVENTS_WARNING_INVALIDIMG'))));
					}*/
					...
					if(!JFile::move($filepathImgTemp, $filepathImgFinal)) {
						...
						'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
						'details' => JTEXT::_('COM_LECTURESEVENTS_ERROR_UNABLE_TO_MOVE_FILE') .'<br />'
						. JText::_('COM_LECTURESEVENTS_CHECK_PERMISSIONS_OWNERSHIP'))));
					}
				...
				'message' => JText::_('COM_LECTURESEVENTS_SUCCESS').': ',
				'details' => JTEXT::_('COM_LECTURESEVENTS_FILES_UPLOADED'))));
				...
				if (!LecturesEventsFileUpload::canUpload( $file, $errUploadMsg, $manager, $frontEnd, $chunkMethod, 0 )) {
					jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'error', 'code' => 104,
					'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
					'details' => JTEXT::_($errUploadMsg))));			
				}
				...
				if(!JFile::upload($file['tmp_name'], $filepathImgFinal)) {
					jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'error', 'code' => 109,
					'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
					'details' => JTEXT::_('COM_LECTURESEVENTS_ERROR_UNABLE_TO_UPLOAD_FILE') .'<br />'
					. JText::_('COM_LECTURESEVENTS_CHECK_PERMISSIONS_OWNERSHIP'))));
				}
				...
				jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'OK', 'code' => 200,
				'message' => JText::_('COM_LECTURESEVENTS_SUCCESS').': ',
				'details' => JTEXT::_('COM_LECTURESEVENTS_IMAGES_UPLOADED'))));
			...
			jexit(json_encode(array( 'jsonrpc' => '2.0', 'result' => 'error', 'code' => 104,
			'message' => JText::_('COM_LECTURESEVENTS_ERROR').': ',
			'details' => JTEXT::_('COM_LECTURESEVENTS_ERROR_UNABLE_TO_UPLOAD_FILE'))));
		}
	...
	//	$uploadMethod 	= $paramsC->get( 'multiple_upload_method', 1 );
		...
		JRequest::checkToken( 'request' ) or jexit( 'ERROR: '. JTEXT::_('COM_LECTURESEVENTS_INVALID_TOKEN'));
		JResponse::allowCache(false);
		...
		if (isset($file['name'])) {
			...
			if (!LecturesEventsFileUpload::canUpload( $file, $errUploadMsg, $manager, $frontEnd )) {
				
				if ($errUploadMsg == 'COM_LECTURESEVENTS_WARNING_FILE_TOOLARGE') {
					$errUploadMsg 	= JText::_($errUploadMsg) . ' ('.LecturesEventsFileUpload::getFileSizeReadable($file['size']).')';
				} /* else if ($errUploadMsg == 'COM_LECTURESEVENTS_WARNING_FILE_TOOLARGE_RESOLUTION') {
					$imgSize		= lectureseventsImage::getImageSize($file['tmp_name']);
			...
			if (JFile::exists($filepath) && $overwriteExistingFiles == 0) {
				if ($return) {
					$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, JText::_('COM_LECTURESEVENTS_FILE_ALREADY_EXISTS'), 'error');
					exit;
				} else {
					$app->redirect($componentUrl, JText::_('COM_LECTURESEVENTS_FILE_ALREADY_EXISTS'), 'error');
					exit;
				}
			}
			...
			if (!JFile::upload($file['tmp_name'], $filepath)) {
				if ($return) {
					$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, JText::_('COM_LECTURESEVENTS_ERROR_UNABLE_TO_UPLOAD_FILE'), 'error');
					exit;
				} else {
					$app->redirect($componentUrl, JText::_('COM_LECTURESEVENTS_ERROR_UNABLE_TO_UPLOAD_FILE'), 'error');
					exit;
				}
			} else {
				...
				if ($return) {
					$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, JText::_('COM_LECTURESEVENTS_SUCCESS_FILE_UPLOAD'));
					exit;
				} else {
					$app->redirect($componentUrl, JText::_('COM_LECTURESEVENTS_SUCCESS_FILE_UPLOAD'));
					exit;
				}
			}
		} else {
			$msg = JText::_('COM_LECTURESEVENTS_ERROR_UNABLE_TO_UPLOAD_FILE');
			if ($return) {
				$app->redirect(base64_decode($return).'&manager='.(string)$manager.'&folder='.$folderUrl, $msg);
				exit;
			} else {
				if($viewBack != '') {
					$group 	= LecturesEventsHelper::getManagerGroup($manager);
					$link	= 'index.php?option=com_lecturesevents&view=lectureseventsmanager&manager='.(string)$manager
							.str_replace('&amp;', '&', $group['c']).'&'.$tabUrl.'&folder='.$folder.'&field='.$field;
					$app->redirect($link, $msg);
				} else {
					$app->redirect('index.php?option=com_lecturesevents', $msg, 'error');
				}
	...
	function canUpload( $file, &$err, $manager = '', $frontEnd = 0, $chunkEnabled = 0, $realSize = 0) {
		...
		if ($frontEnd == 1) {
			$aft = $paramsC->get( 'allowed_file_types_upload', LecturesEventsHelper::getDefaultAllowedMimeTypesUpload() );
			$dft = $paramsC->get( 'disallowed_file_types_upload', '' );
			$allowedMimeType 	= LecturesEventsHelper::getMimeTypeString($aft);
			$disallowedMimeType = LecturesEventsHelper::getMimeTypeString($dft);
		} else {
		
			$aft = $paramsC->get( 'allowed_file_types_download', LecturesEventsHelper::getDefaultAllowedMimeTypesDownload() );
		...
		if(empty($file['name'])) {
			$err = 'COM_LECTURESEVENTS_WARNING_INPUT_FILE_UPLOAD';
			return false;
		}
		// Not safe file
		jimport('joomla.filesystem.file');
		if ($file['name'] !== JFile::makesafe($file['name'])) {
			$err = 'COM_LECTURESEVENTS_WARNFILENAME';
			return false;
		}
		...
		if(in_array($format, $notAllowable)) {
			$err = 'COM_LECTURESEVENTS_WARNFILETYPE_DISALLOWED';
			return false;
		}
		
		
		if (!in_array($format, $allowable)) {
			$err = 'COM_LECTURESEVENTS_WARNFILETYPE_NOT_ALLOWED';
			return false;
		}
		...
		if ($chunkEnabled == 1) {
			if ((int)$maxSize > 0 && (int)$realSize > (int)$maxSize) {
				$err = 'COM_LECTURESEVENTS_WARNFILETOOLARGE';
		...
		if ($frontEnd == 2) {
			...
			$allFile	= LecturesEventsHelper:: getUserFileInfo($file, $user->id);
			...
			if ((int)$maxUserUploadSize > 0 && (int) $allFile['size'] > $maxUserUploadSize) {
				$err = JText::_('COM_LECTURESEVENTS_WARNUSERFILESTOOLARGE');	
				return false;
			}
				
			if ((int) $allFile['count'] > $maxUserUploadCount) {
				$err = JText::_('COM_LECTURESEVENTS_WARNUSERFILESTOOMUCH');	
				return false;
			}
		}
		...
		if(in_array($format, $images)) { // if its an image run it through getimagesize
			...
			if($group['i'] == 1) {
				if ($chunkEnabled != 1) {
					if(($imginfo = getimagesize($file['tmp_name'])) === FALSE) {
						$err = 'COM_LECTURESEVENTS_WARNINVALIDIMG';
						$err = $imginfo[0];
						return false;
					}
				}
			...
			if(function_exists('finfo_open')) {// We have fileinfo
				...
				if(strlen($type) && !in_array($type, $allowed_mime) && in_array($type, $illegal_mime)) {
					$err = 'COM_LECTURESEVENTS_WARNINVALIDMIME';
					return false;
				}
				finfo_close($finfo);
	...
	function renderFTPaccess() {
		...
		$ftpOutput = '<fieldset title="'.JText::_('COM_LECTURESEVENTS_FTP_LOGIN_LABEL'). '">'
		.'<legend>'. JText::_('COM_LECTURESEVENTS_FTP_LOGIN_LABEL').'</legend>'
		.JText::_('COM_LECTURESEVENTS_FTP_LOGIN_DESC')
	...
	function renderCreateFolder($sessName, $sessId, $currentFolder, $viewBack, $attribs = '') {
		...
		.'index.php?option=com_lecturesevents&task=lectureseventsupload.createfolder&amp;'. $sessName.'='.$sessId.'&amp;'
		...
		.'<legend>'.JText::_('COM_LECTURESEVENTS_FOLDER').'</legend>'."\n"
		.'<div class="path">'
		.'<input class="inputbox" type="text" id="foldername" name="foldername"  />'
		.'<input class="update-folder" type="hidden" name="folderbase" id="folderbase" value="'.$currentFolder.'" />'
		.'<button type="submit">'. JText::_( 'COM_LECTURESEVENTS_CREATE_FOLDER' ).'</button>'
		.'</div>'."\n"
	    .'</fieldset>'."\n"
		.JHTML::_( 'form.token' )
		.'</form>';
